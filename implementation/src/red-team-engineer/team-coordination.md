...
*   **Path:** `results/red-team/phase1-baseline/`
*   **Contents to be uploaded:**
    *   `operation_report.pdf` (Executive summary of the attack)
    *   `operation_report.json` (Raw data for the Threat Hunter to parse)
    *   `attack_graph.svg` (Visual representation of the attack path)
    *   `mitre_matrix.json` (TTP coverage export)

**Note:** Physical files will be synchronized to the repository once the lab environment export is complete.